//여러개 주문이 안되고 한번에 한 식자재만, 한개씩 주문 가능(여러개 담으면 처음에 담은 것만 주문됨)
<template>
    <v-app>
      <v-container fluid>
        <v-row>
...
export default {
  ...
  methods: {
    ...
    calculateTotal() {
      this.cartTotal = this.cart.reduce((total, item) => total + item.storeCommPrice, 0);
    },
    async submitOrder() {
      try {
        // 주문 요청 데이터 생성
        const orderData = {
          ...
          //storeCommId: 3,
          //storeOrderDetailQuantity: 1 // 예시로 1로 설정합니다. 주문 수량을 설정할 수 있습니다.
          storeCommId: this.cart[0].storeCommId,
          storeOrderDetailQuantity: 1

          
        };

        // DB에 주문 생성 요청 보내기
        await axios.post('http://localhost:8080/api/v1/storecomms/order', orderData);
    ...
    async fetchStoreComms() {
      try {
        ...
        this.supplyItems = items.filter(item => item.storeCommCategory === 'SUPPLY');
      } catch (error) {
