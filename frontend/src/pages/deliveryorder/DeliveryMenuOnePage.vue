  ...
  <v-container fluid>
    ...
    <v-row class="justify-center">
      <v-col cols="12" md="6" class="text-center">
        <v-img
          class="mx-auto my-4"
          :src="'https://ourpos.org/images/' + menu.pictureUrl"
          max-height="228"
        ></v-img>
...
const findMenu = async () => {
  try {
    const response = await axios.get(`https://ourpos.org/api/v1/menus/${menuId}`, {
...
const findMenuOptionGroups = async () => {
  try {
    const response = await axios.get(`https://ourpos.org/api/v1/categories/${menu.value.categoryId}`, {
      withCredentials: true
    });
