<template>
  <div class="box_section">
    <p class="title payment-widget-cache-bd75t9">{{ orderData.storeName }} - 배달 주문하기</p>
    <p>
      <FontAwesomeIcon :icon="faMapSigns" />
      {{ orderData.orderAddressRequestDto.addressBase }}
    </p>
    <!-- 결제 UI -->
...
const loadOrderData = () => {
  const data = localStorage.getItem('deliveryOrder');
  if (data) {
...
const requestPayment = async () => {
  try {
    ...
    if (response.code === 200) {
      ...
      router.push(`/fail?message=${response.message}&code=${response.code}`);
      return;
    }
    ...
    router.push(`/fail?message=${error.message}&code=500`);
  }
};
...
const checkForChanges = () => {
  const initialData = localStorage.getItem('deliveryOrder');
  window.addEventListener('storage', () => {
    const currentData = localStorage.getItem('deliveryOrder');
...
onMounted(async () => {
  ...
  if (orderData.value.orderDetailDtos.length === 0) {
    alert('주문할 상품이 없습니다.');
    router.push('/stores/delivery');
    return;
  }
