...
export default {
  ...
  methods: {
    async loadAddresses() {
      try {
        const response = await axiosInstance.get('https://api.ourpos.org/api/v1/customers/my/addresses');
    ...
    async updateSubAddress(index) {
      try {
        ...
        // 서브 주소를 기본 주소로 변경
        await axiosInstance.put(`https://api.ourpos.org/api/v1/customers/addresses/${addressId}/default`);
    ...
    async saveSubAddress() {
      try {
        ...
        await axiosInstance.put(`https://api.ourpos.org/api/v1/customers/addresses/${addressId}`, this.tempSubAddress);
    ...
    async deleteSubAddress(addressId) {
      try {
        await axiosInstance.delete(`https://api.ourpos.org/api/v1/customers/addresses/${addressId}`);
    ...
    async saveNewSubAddress() {
      ...
      try {
        if (this.newSubAddress.addressBase) {
          await axiosInstance.post('https://api.ourpos.org/api/v1/customers/addresses', this.newSubAddress);
          await this.loadAddresses();
