...
const getMenuImageUrl = (imagePath) => {
  return `https://ourpos.org/images/${imagePath}`;
};
...
const addCategory = async () => {
  try {
    const response = await axios.post('https://ourpos.org/api/v1/categories', {
...
const editCategory = async () => {
  try {
    ...
    const response = await axios.put(`https://ourpos.org/api/v1/categories/${categoryId}/update`, {
...
const deleteCategory = async () => {
  try {
    ...
    const response = await axios.put(`https://ourpos.org/api/v1/categories/${categoryId}/delete`);
    if (response.status === 200) {
...
const addMenu = async () => {
  ...
  try {
    const response = await axios.post('https://ourpos.org/api/v1/menus', formData, {
      headers: {
...
const deleteMenu = async () => {
  try {
    const response = await axios.put(`https://ourpos.org/api/v1/menus/${selectedMenu.value.id}/delete`);
    if (response.status === 200) {
      fetchMenus();
      closeDialog();
    } else {
